// src/features/hero/HeroSlide.tsx
...
export default function HeroSlide({
  title, subtitle,
  timeLabel, currentBid, lotsLabel, auctionId,
  image, primaryHref, onHowToBid,
}: HeroSlideProps) {
  return (
    <div className="relative">
      ...
      <div className="absolute inset-x-0 bottom-0 mx-auto max-w-7xl px-4 sm:px-6 pb-10">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-2 text-white/90">
          <span className="rounded-full bg-[#E02020] px-3 py-1 text-sm font-semibold">{timeLabel}</span>
          <span className="rounded bg-white/15 backdrop-blur px-2.5 py-1 text-sm font-semibold">Current Bid: {currentBid}</span>
          <span className="text-sm/6 opacity-90">• {lotsLabel}</span>
          <span className="text-sm/6 opacity-70">• {auctionId}</span>
        </div>

        {subtitle && (
          <p className="mt-4 max-w-3xl text-white/90 line-clamp-2 md:line-clamp-3">{subtitle}</p>
        )}
        ...
        <div className="mt-6 flex flex-wrap items-center gap-3">
          ...
          <button
            onClick={onHowToBid}
            className="inline-flex items-center rounded-2xl border border-white/25 bg-white/10 px-5 py-3 font-semibold text-white/95 backdrop-blur
                       hover:bg-white/15"
          >
            How to Bid
          </button>
