...
export default function HeroRotator({ slides, autoplayMs = 3600, transitionMs = 600, className = "" }: Props){
  ...
  // Start/stop autoplay only when visible
  function restart(){
  ...
  useEffect(()=>{
    ...
    if(!el) return;
    const io = new IntersectionObserver(([e]) => { visibleRef.current = !!e?.isIntersecting; restart(); }, { threshold: 0.25 });
    io.observe(el);
